...
### Source Access Is Intent-Driven

The platform does not treat "can this caller read source?" as a generic bucket question.

`sourceAccess` shapes the response by intent:
...
That keeps the public story honest. A Studio edit surface is not the same as a clone/export surface, and neither is the same as a deploy or operator surface.

### Authored Writes Are Backend-Owned

The platform also does not let the client invent authored-path identity.

`capsuleFiles` and `authoredLayout` decide whether a capsule is still in legacy-preserve mode or has moved to standardized authored paths. The backend returns the path truth it accepted.
...
The direct artifact host now uses a dedicated public mirror lane. The canonical artifact bucket is not the public bucket.
...
Presigned URLs are useful, but they are not the universal abstraction for this system.
...
- explicit mirror and warmup lanes for public runtime artifacts
...
- avatars and covers can be cleaned up when orphaned
...
Deduplicated blobs and dependencies can be physically shared while still being logically charged or ref-counted in a way that matches product expectations.
